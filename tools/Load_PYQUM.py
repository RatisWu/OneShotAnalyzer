...
def command_analytic(selectdata,corder, perimeter,datadensity):
    ...
    if 'READOUTYPE' in perimeter.keys():
        ...
        for key, value in RJSON.items():
            print('\t',key, ' : ', value)
        if 'IF_ALIGN_KHZ' in perimeter :
            if len(perimeter['IF_ALIGN_KHZ'].split(" ")) > 1:
                command['change'] = append(command['change'],'IF_ALIGN_MHz')
                command['change_command'] = append(command['change_command'],perimeter['IF_ALIGN_KHZ'])
                command['change_len'] = append(command['change_len'],len(perimeter['IF_ALIGN_KHZ'].split(" ")))
                print("IF_ALIGN_MHz: ",str(perimeter['IF_ALIGN_KHZ'].split(" ")))
        if perimeter['READOUTYPE'] == 'continuous':
            shot = int(perimeter['RECORD-SUM'])
            print('RECORD-SUM : ',shot)
            time_unit = int(perimeter['TIME_RESOLUTION_NS'])
